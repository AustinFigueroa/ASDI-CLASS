...
ui.button.addEventListener("click", function(e){
	if(ui.button.edit != true) {
		...
		ui.manaField.blur();
		ui.lvlField.blur();
		data.create(newTitle, newHealth, newMana, newLvl);
		data.read();
	} else {
		ui.tableData.removeAllChildren();
		data.upload(ui.button.index, ui.button.location);
	}
});
...
ui.tableData.addEventListener("click", function(e){
	...
	dialog.addEventListener("click", function(evt){
		...
		if (evt.index === 1) {
			...
			confrimDelete.addEventListener("click",function(evt2){
				if (evt2.index === 0){
					ui.button.id = e.row.id;
